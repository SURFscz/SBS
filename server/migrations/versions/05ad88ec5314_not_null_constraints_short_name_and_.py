...
def upgrade():
    ...
    result = conn.execute("SELECT id FROM `organisations` WHERE `name` IS NULL")
    for row in result:
        id = row["id"]
        conn.execute(f"UPDATE `organisations` SET `name` = '{random_string()}' WHERE id = {id}")

    result = conn.execute("SELECT id, `name` FROM `organisations` WHERE short_name IS NULL")
    for row in result:
        id = row["id"]
        name = row["name"]
        conn.execute(f"UPDATE `organisations` SET short_name = '{cleanse_short_name(name)}' WHERE id = {id}")
    ...
    result = conn.execute("SELECT id, name FROM `collaborations` WHERE short_name IS NULL")
    for row in result:
        id = row["id"]
        name = row["name"]
        conn.execute(f"UPDATE `collaborations` SET short_name = '{cleanse_short_name(name)}' WHERE id = {id}")

    result = conn.execute("SELECT coll.id AS id, coll.short_name AS coll_short_name, org.short_name AS org_short_name "
                          "FROM "
    ...
    for row in result:
        ...
        conn.execute(f"UPDATE `collaborations` SET global_urn = '{global_urn}' WHERE id = {id}")
    ...
    result = conn.execute("SELECT gr.id AS id, gr.short_name AS gr_short_name, coll.short_name AS coll_short_name, "
    ...
    for row in result:
        ...
        conn.execute(f"UPDATE `groups` SET global_urn = '{global_urn}' WHERE id = {id}")
