...
def broadcast_organisation_service_added(organisation_id: int, service_id: int):
    return current_app.executor.submit(apply_organisation_change, current_app, organisation_id, service_id)
...
def broadcast_collaboration_changed(collaboration_id: int):
    return current_app.executor.submit(apply_collaboration_change, current_app, collaboration_id)
...
def broadcast_group_changed(group_id: int):
    return current_app.executor.submit(apply_group_change, current_app, group_id)
...
def broadcast_service_added(collaboration_id: int, service_id: int):
    return current_app.executor.submit(apply_service_changed, current_app, collaboration_id, service_id)
